import 'dart:convert';
import 'dart:ffi';
import 'package:frontend/edit_ordre.dart';
import 'package:frontend/pages/login/admin/ajouter/ajouter_sav.dart';
...
class Admin_Details extends StatefulWidget {
  final id;
  Admin_Details({super.key, this.id});

  @override
  State<Admin_Details> createState() => Admin_DetailsState();
}

class Admin_DetailsState extends State<Admin_Details> {
  late Map dataList = {};
  ...
  recover() async {
    var response = await http
        .get(Uri.parse("https://sirinelec.be/api/RecoverOrdre/${widget.id}"));
  }
  ...
  // sendImageToServer(int ordreId) async {
  //   final picker = ImagePicker();
  //   final pickedFile = await picker.getImage(
  //       source:
  //           ImageSource.camera); // Use ImageSource.camera for capturing a photo

  //   if (pickedFile != null) {
  //     // Create a FormData object to send the image
  //     final formData = http.MultipartRequest(
  //         'POST', Uri.parse('https://sirinelec.be/api/UploadPic/$ordreId'));
  //     formData.files
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return SpeedDial(
                    ...
                    children: [
                      ...
                      SpeedDialChild(
                          child: Icon(
                              (dataList["deleted"] == false)
                                  ? Icons.delete
                                  : Icons.refresh,
                              color: Colors.white),
                          label: (dataList["deleted"] == false)
                              ? 'Suprimmer'
                              : "Recouvrir",
                          backgroundColor: Colors.green,
                          onTap: (dataList["deleted"] == false)
                              ? () {
                                  // ElevatedButton(
                ...
                Expanded(
                    child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Add a default return statement here (e.g., an empty Container)
                          return ListTile(
                            title: Text(key),
